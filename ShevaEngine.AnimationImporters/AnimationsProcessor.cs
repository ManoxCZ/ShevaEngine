...
namespace AnimationImporters
{
	...
	public class AnimationsProcessor : ContentProcessor<NodeContent, AnimationsContent>
	{
		...
		[DefaultValue(30)] 
		public int GenerateKeyframesFrequency { get; set; } = 30;

		[DisplayName("Fix BoneRoot from MonoGame importer")]
		[DefaultValue(false)]
		public bool FixRealBoneRoot { get; set; } = false;

		[DisplayName("Flatten mesh")]
		[DefaultValue(false)]
		public bool FlattenMesh { get; set; } = false;
		...
		/// add the real bone root to the root node.
		/// </summary>
		...
		/// </summary>
		Dictionary<string, ClipContent> ProcessAnimations(NodeContent input, ContentProcessorContext context, 
		...
		/// object to our runtime AnimationClip format.
		/// </summary>
		private ClipContent ProcessAnimation(NodeContent input, ContentProcessorContext context, 
			AnimationContent animation, Dictionary<string, int> boneMap, int generateKeyframesFrequency)
		{
			int frames = (int)Math.Floor(animation.Duration.TotalSeconds * 30); 

			Matrix[][] keyFrames = new Matrix[frames][];

			for (int i = 0; i < frames; i++)
			{
				keyFrames[i] = new Matrix[boneMap.Count];

				for (int j = 0; j < boneMap.Count; j++)
					keyFrames[i][j] = Matrix.Identity;				
			}
			...
			foreach (KeyValuePair<string, AnimationChannel> channel in animation.Channels)
			{
				...
				for (int i = 0; i < frames; i++)
					keyFrames[i][boneIndex] = channel.Value[Math.Min(i, channel.Value.Count - 1)].Transform;
			}

			return new ClipContent()
			{
		...
		/// </summary>
		private static int CompareKeyframeTimes(KeyframeContent a, KeyframeContent b)
		{
		...
		private List<KeyframeContent> InterpolateKeyframes(TimeSpan duration, List<KeyframeContent> keyframes, int generateKeyframesFrequency)
		{
			...
			// check if keyframes are already fully interpolated
